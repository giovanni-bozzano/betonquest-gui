...
public class WidgetText extends AbstractGui implements IRenderable
{
    ...
    public WidgetText(ITextComponent text, int x, int y, double scale)
    {
        this.text = text;
        this.x = x;
        this.y = y;
    ...
    public WidgetText(ITextComponent text, int x, int y)
    {
        this(text, x, y, 1.0F);
    }
    ...
    public void render(int mouseX, int mouseY, float unused)
    {
        ...
        this.drawString(Minecraft.getInstance().fontRenderer, this.text.getFormattedText(), (int) (this.x / this.scale), (int) (this.y / this.scale), TextFormatting.WHITE.getColor());
